...
const IV_LEN = 12;
const KEY_LEN = 32;
export function createSessionCrypto(secrets) {
    const rawKeys = (Array.isArray(secrets) ? secrets : [secrets]);
    if (rawKeys.length === 0) {
    ...
    return {
        async encrypt(plaintext) {
            ...
            const ct = await crypto.subtle.encrypt({ name: "AES-GCM", iv: toBufferSource(iv) }, key, toBufferSource(new TextEncoder().encode(plaintext)));
        ...
        async decrypt(envelope) {
            ...
            for (const keyPromise of keyPromises) {
                try {
                    ...
                    const pt = await crypto.subtle.decrypt({ name: "AES-GCM", iv: toBufferSource(iv) }, key, toBufferSource(ct));
...
function decodeSecret(secret) {
    ...
    for (const dec of tryDecoders) {
        ...
        if (bytes && bytes.byteLength === KEY_LEN)
            return bytes;
    }
    throw new Error(`session secret must decode to ${KEY_LEN} bytes (base64url, base64, or hex)`);
}
...
async function importKey(raw) {
    return crypto.subtle.importKey("raw", toBufferSource(raw), { name: "AES-GCM" }, false, ["encrypt", "decrypt"]);
}
/**
