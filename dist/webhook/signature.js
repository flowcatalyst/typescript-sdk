 ...
 */
export function verifyDeliverySignature(params) {
    const { rawBody, secret } = params;
    const tolerance = params.toleranceSeconds ?? 300;
    if (!secret) {
        throw new WebhookSignatureError("missing_secret", "Webhook signing secret is not configured.");
    }
    const signature = single(params.signature);
    if (!signature) {
        throw new WebhookSignatureError("missing_signature", "Missing X-FlowCatalyst-Signature header.");
    }
    const timestamp = single(params.timestamp);
    if (!timestamp) {
        throw new WebhookSignatureError("missing_timestamp", "Missing X-FlowCatalyst-Timestamp header.");
    }
    const webhookSeconds = parseTimestamp(timestamp);
    if (webhookSeconds === null) {
        throw new WebhookSignatureError("invalid_timestamp", `Unparseable X-FlowCatalyst-Timestamp '${timestamp}'.`);
    }
    const nowSeconds = Math.floor(Date.now() / 1000);
    if (webhookSeconds < nowSeconds - tolerance) {
        throw new WebhookSignatureError("timestamp_expired", `Delivery timestamp older than ${tolerance}s — replay rejected.`);
    }
    if (webhookSeconds > nowSeconds + 60) {
        throw new WebhookSignatureError("timestamp_in_future", "Delivery timestamp is in the future — check clock sync.");
    }
    ...
    if (params.expectedBearerToken) {
        ...
        if (!auth || !auth.startsWith(prefix)) {
            throw new WebhookSignatureError("missing_bearer", "Missing Authorization bearer token.");
        }
