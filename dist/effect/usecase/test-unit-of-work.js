...
export const layer = (recorder) => Layer.succeed(UnitOfWork, {
    commit: (event, _command, _persist) => Effect.sync(() => {
        recorder.push(event);
        return seal(event);
    }),
    commitDelete: (_aggregate, event, _command, _persist) => Effect.sync(() => {
        recorder.push(event);
        return seal(event);
    }),
    emitEvent: (event, _command) => Effect.sync(() => {
        recorder.push(event);
        return seal(event);
    }),
});
