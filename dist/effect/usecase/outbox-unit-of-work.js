...
const toEventDto = (event, clientCode) => {
...
const toAuditDto = (event, command, fallbackPrincipalId, applicationCode, clientCode) => {
...
export const layer = (outboxManager, options) => {
    ...
    const doCommit = (event, command, persist) => Effect.tryPromise({
        try: async () => {
            ...
            if (auditEnabled) {
                await outboxManager.createAuditLog(toAuditDto(event, command, fallbackPrincipalId, applicationCode, clientCode));
            }
            return seal(event);
        },
