...
export class DefinitionSetBuilder {
     ...
     */
    forClient(client, targetBaseUrl) {
 ...
 */
export function defineApplicationFromEnv() {
    const applicationCode = typeof process !== "undefined" ? process.env?.[APP_CODE_ENV] : undefined;
    if (!applicationCode) {
        throw new Error(`${APP_CODE_ENV} is not set — pass the application code to defineApplication() instead.`);
    }
    return new DefinitionSetBuilder(applicationCode);
}
