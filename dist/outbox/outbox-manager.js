...
export class OutboxManager {
    ...
    async createDispatchJob(job, tx) {
        this.ensureClientId();
        const id = generate();
    ...
    async createDispatchJobs(jobs, tx) {
        if (jobs.length === 0)
            return [];
        this.ensureClientId();
        const ids = [];
    ...
    async createAuditLog(auditLog, tx) {
        this.ensureClientId();
        const id = generate();
    ...
    async createAuditLogs(auditLogs, tx) {
