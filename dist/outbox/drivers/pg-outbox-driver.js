...
`;
const isPool = (executor) => typeof executor.connect === "function";
const isPoolClient = (tx) => typeof tx === "object" &&
    tx !== null &&
...
export class PgOutboxDriver {
     ...
     */
    async insert(message, tx) {
        const client = isPoolClient(tx) ? tx : this.executor;
    ...
    async withTransaction(callback) {
        if (!isPool(this.executor)) {
            throw new Error("PgOutboxDriver.withTransaction requires a pool-like executor with `connect()`. Construct the driver with a `pg.Pool` to enable orchestrated transactions.");
        }
        const client = await this.executor.connect();
        try {
            await client.query("BEGIN");
